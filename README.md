...
SwiftDataはSwiftUIとの親和性が高く、比較的シンプルなデータ管理に適していますが、将来的にデータ件数の増加や検索・並び替え機能の追加を想定したため、
本アプリではより多くのデータを安定して扱えるCoreDataを使用しています。
...
### 技術面でアドバイスして欲しいポイント
今回のようなお気に入りリストの永続化については、SwiftDataまたはCoreDataを採用するのが一般的だと認識していますが、
データ規模や将来的な拡張性、パフォーマンス要件などを踏まえ、実務において両者をどのような基準で使い分けているのか、設計判断の考え方についてアドバイスをいただきたいです。

### 自己評価
- 良かった点
 ・比較的早い段階で最低限の機能を実装できたこと。
 ・シンプルで見やすい UI・UX に仕上げられたこと。
 ・拡張機能を複数実装し、エラー処理にも対応できたこと。
 
- 改善点
 ・コードのリファクタリングに想定以上の時間を要したこと。
 ・複数の View のロジックを 1 つの ViewModel にまとめて実装してしまったこと。
   当初はコード量が少ないと判断し 1 ファイルで管理していたが、結果として可読性や保守性が低下したため、途中から View ごとに ViewModel を分割する構成に修正した。
 ・位置情報の利用を拒否された場合の挙動を考慮できておらず、後から例外処理を追加したこと。

- まとめ
  今後は、初期設計の段階で責務分離や例外ケースをより明確に洗い出し、後戻りの少ない構成を意識した開発を行いたいと考えています。
